...
const AccountSection: FunctionComponent<{ user: User | null }> = (props) => {
	...
	const [userState, setUserState] = useState<Pick<User, 'username' | 'fullName' | 'email'>>();
	const [isSaving, setIsSaving] = useState<boolean>(false);

	useEffect(() => {
		if (user != null && userState == null) {
			setUserState(pick(user, ['username', 'fullName', 'email']));
		}
	}, [user]);
					...
					{userLoaded && (
						<>
							...
							<CardBody pad="medium" gap="medium">
								...
								<FormField name="fullName" label="Name" contentProps={{ width: '80%' }}>
